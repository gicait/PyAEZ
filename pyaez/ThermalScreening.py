...
class ThermalScreening(object):
    ...
    def getTemperatureProfile(self, start_day, cycle_len):
        ...
        interp1D = np.zeros(temp1D.size)

        # start 5th order polynomical interpolation
        days = np.arange(temp1D.size)

        #1 test = np.polyfit(days, temp1D, 5)
        #1 quadspl = np.poly1d(test)

        #1 interp1D = quadspl(days)
        plyfit = Polynomial.fit(days, temp1D, deg=5, window = [np.min(temp1D), np.max(temp1D)])

        interp1D = plyfit(days).astype('float32')
        ...
        # releasing memory
        del (temp1D, interp1D, days, plyfit,
             meanT_daily_add1day, meanT_first, meanT_diff)
