...
def calculateBiomassNumba(cycle_begin:int, cycle_end:int, cycle_len:int, latitude:float,
    ...
    Ac_mdl = CubicSpline(new_doy, new_Ac_interp1)
    Bc_mdl = CubicSpline(new_doy, new_Bc_interp1)
    Bo_mdl = CubicSpline(new_doy, new_Bo_interp1)


    Ac_interp  = Ac_mdl(np.arange(cycle_begin, cycle_end))
    Bc_interp  = Bc_mdl(np.arange(cycle_begin, cycle_end))
    Bo_interp  = Bo_mdl(np.arange(cycle_begin, cycle_end))
...
#     return f0
 ...
 # ---------------------------------------------------------- Developer's Codes ---------------------------------------------------------------------#
def calculateBiomassNumbaIntermediates(cycle_begin:int, cycle_end:int, cycle_len:int, latitude:float,
                         shortRad_daily, meanT_daily, minT_daily, maxT_daily,
                        LAi:float, legume:int, adaptability:int, leap_year:bool):
