...
def calculateETONumba(cycle_begin, cycle_end, latitude, alt,  minT_daily, maxT_daily, windspeed_daily, shortRad_daily, rel_humidity):
    ...
    for i in range(dayhr.shape[0]):
        ...
        if abs(zz[i]) >= 0.9999:
            ...
            omg[i] = np.arctan(zz[i]/ np.sqrt(1.- (zz[i] * zz[i]))) + 1.507
    ...
    eto_l = 0.408 * dl * (rn - G)
    eto_r = gam * (900./(tavg + 273.15)) * u2m * (ea - ed)
    eto_d = dl + (gam * (1 + (0.34 * u2m)))

    et0 = (eto_l + eto_r) / eto_d
    # et0ady = (gam/(dl+gamst)) * gamst * (900./(tavg+273.)) * u2m * (ea-ed)
...
@nb.jit(nopython=True)
def calculateNetRadiationFlux(cycle_begin, cycle_end, latitude, alt,  minT_daily, maxT_daily, shortRad_daily, rel_humidity, wind_sp):
